          ...
          <tbody>
            {enrollments.map((enrollment) => (
              <tr key={enrollment.id}>
                ...
                <td>
                  <form action={reviewEnrollment} className={styles.actionRow} style={{ flexWrap: "nowrap" }}>
                    <input type="hidden" name="enrollmentId" value={enrollment.id} />
                    <select
                      name="status"
                      className={styles.select}
                      defaultValue={enrollment.status}
                      style={{ width: "auto", minWidth: "7.25rem" }}
                    >
                      <option value="PENDING">Pending</option>
                      <option value="APPROVED">Approve</option>
                      <option value="REJECTED">Reject</option>
                    </select>
                    <button className={styles.button} type="submit" style={{ width: "auto", whiteSpace: "nowrap", paddingInline: "0.9rem" }}>
                      Update
                    </button>
                  </form>
                </td>
